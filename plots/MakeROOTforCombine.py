...
savefileheader = '../outputs/combine/TTbarAllHadFull_'
...
for IOV in IOVs:
    ...
    for cat, catname in zip(cats, cat_labels):
        ...
        froot["bkgest_"+catname+'_mistagUp_'+IOV] = HistDict['ntmj_fixed'] + mistagErrProp
        froot["bkgest_"+catname+'_mistagDown_'+IOV] = HistDict['ntmj_fixed'] + -1.0*mistagErrProp

        froot["bkgest_"+catname+'_massmodUp_'+IOV] = HistDict['ntmj_fixed'] + mmErrProp
        froot["bkgest_"+catname+'_massmodDown_'+IOV] = HistDict['ntmj_fixed'] + -1.0*mmErrProp

        froot["bkgest_"+catname+'_qcdclosureUp_'+IOV] = HistDict['ntmj_fixed'] + qcdErrProp
        froot["bkgest_"+catname+'_qcdclosureDown_'+IOV] = HistDict['ntmj_fixed'] + -1.0*qcdErrProp
        ...
        for syst in syst_labels:
            ...
            if '2016all' in IOV:
                ...
                if ('prefiring' in syst) and (IOV == '2018'):
                    ...
                    catsystString = catname+'_'+syst+'_'
                ...
                if 'nominal' in syst:
                    ...
                    hntmj_fixed = hntmj + -1*hcontam
                    print('data_obs_'+catsystString+IOV+'\nbkgest_'+catsystString+IOV)
                    froot["data_obs_"+catsystString+IOV] = hdata
                    froot["bkgest_"+catsystString+IOV] = hntmj_fixed

                print('TTbar_'+catsystString+IOV)
                if 'prefiring' in syst and IOV == '2018':
                    continue
                else:
                    froot["TTbar_"+catsystString+IOV] = httbar
                        ...
                        if 'RSGluon' not in sig:
                            if sig != 'ZPrime1': # ZPrime10, 30 and DM
                                if 'prefiring' in syst and IOV == '2018':
                                    ...
                                    [print(sig[:-2]+mass+'_'+sig[-2:]+'_'+catsystString+IOV) for mass in Signals[sig]]
                                    for mass in Signals[sig]:
                                        froot[sig[:-2]+mass+'_'+sig[-2:]+'_'+catsystString+IOV] = hsignal[sig][mass]
                            else: # ZPrime1
                                if 'prefiring' in syst and IOV == '2018':
                                    continue
                                else:
                                    [print(sig[:-1]+mass+'_'+sig[-1:]+'_'+catsystString+IOV) for mass in Signals[sig]]
                                    for mass in Signals[sig]:
                                        froot[sig[:-1]+mass+'_'+sig[-1:]+'_'+catsystString+IOV] = hsignal[sig][mass]
                        else: # RSGluon
                            if 'prefiring' in syst and IOV == '2018':
                                continue
                            else:
                                [print(sig+mass+'_'+catsystString+IOV) for mass in Signals[sig]]
                                for mass in Signals[sig]:
                                    froot[sig+mass+'_'+catsystString+IOV] = hsignal[sig][mass]
